"""
Run the MedRecon V3 agent pipeline across all synthetic evaluation cases.

Purpose:
    Produce reproducible predictions from the V3 agent pipeline.

V3 adds:
    Medication interaction screening after reconciliation.
...
Why this is a separate V3 runner:
    Earlier experiments remain preserved for transparent comparison:

        V0 baseline
        V1 first agent pipeline
        V2 improved reconciliation pipeline
        V3 reconciliation + interaction screening

Outputs:
    outputs/evaluations/agent_v3/SYN-XXX.json
...
def main() -> None:
    """
    Run MedRecon V3 across every synthetic case.

    Workflow:
        1. discover all SYN-XXX case files
        2. run the V3 orchestrator
        3. save each case result separately
        4. save one combined V3 result file
