"""
Evaluate the MedRecon AI V1 agent pipeline against synthetic ground truth.

Purpose:
    Measure whether the V1 agentic medication reconciliation workflow
    improves over the frozen V0 deterministic baseline.
...
Important:
    This evaluator uses the same comparison rules as the V0 baseline
    evaluator so the V0 and V1 results remain directly comparable.
...
def precision_recall_f1(
    ...
    Args:
        predicted:
            Predictions produced by the V1 pipeline.
...
def evaluate_case(
    ...
    """
    Evaluate one V1 pipeline prediction against one ground-truth case.
...
def main() -> None:
    """
    Evaluate all successful MedRecon V1 pipeline predictions.
    """
    if not AGENT_RESULTS_FILE.exists():
        raise FileNotFoundError(
            "V1 agent results were not found. "
            "Run scripts/run_agent_pipeline.py first."
        )
    ...
    if not predictions:
        raise ValueError(
            "The V1 agent results file contains no predictions."
        )
    ...
    if v1_f1 > 0.6:
        print(
            "Result: V1 improved over V0."
        )

    elif v1_f1 == 0.6:
        print(
            "Result: V1 matched V0."
        )

    else:
        print(
            "Result: V1 is currently below V0."
        )
